...
public class TokenBlackListFilter implements WebFilter {
    ...
    public TokenBlackListFilter(TokenFinder tokenFinder,
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        log.info("Entering in the token black list filter ");
...
//            return remoteConfigService.getConfig().flatMap(stringStringMap -> {
//                log.info("calling config service {}", stringStringMap);
//                return bearer.commence(exchange,
//                        new OAuth2AuthenticationException("Token is in black list"));
//            });
            return bearer.commence(exchange,
                    new OAuth2AuthenticationException("Token is in black list"));
        } else {
